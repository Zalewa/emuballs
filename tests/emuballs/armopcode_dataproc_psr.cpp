...
#include "src/emuballs/armopcode_dataproc_psr.hpp"
#include "arm_program_fixture.hpp"

using namespace Emuballs::Arm::OpcodeImpl;
...
static Emuballs::Arm::Flags &testCaseFlags(Emuballs::Arm::Machine &machine, const std::string &psr)
{
	if (psr == "cpsr")
	{
...
BOOST_AUTO_TEST_CASE(msr_imm)
{
	...
	for (auto &testCase : testSet)
	{
		Emuballs::Arm::regval imm = (testCase.instruction & 0xff)
